...
## 1. Background
Generative embedders utilizing Decoder-Only LLMs via Last-Token Pooling face a critical issue: Next-Token Prediction (NTP) geometric biases produce anisotropic semantic spaces, degrading retrieval performance. 

Post-hoc SVD representation refinement, such as EmbFilter (Chen et al., 2026), mitigates this by discarding extreme singular value components. However, EmbFilter assumes the highest singular values (Head Spectrum) exclusively contain high-frequency stopword noise. While effective for English, this assumption is untested for agglutinative languages. Because BPE tokenizers fragment bound morphemes into subwords (Bostan et al., 2023), their spectral behavior likely diverges from English stopwords. This study investigates Indonesian morphological energy distribution and proposes a tailored SVD retention window.
...
## 4. Discussion
The results strongly challenge the core assumption of EmbFilter for multilingual contexts. While Chen et al. (2026) considered the Head Spectrum as pure stopword noise, our `Indonesian-Retention` configuration achieves a superior NDCG@10 of 0.2900 precisely by *retaining* this Head zone—outperforming the `English-Middle` filter by +24.3%. 

This indicates that spectral noise distribution inverts in multilingual joint vocabularies. Structural markers dominate the high-variance Head, pushing common English stopwords into the low-variance Tail. Because Indonesian encodes critical syntactic functions through bound affixes (e.g., `meng-`, `-nya`), these fragmented morphemes deposit 75–80% of their latent energy in the Head and Middle spectra. Truncating the Head inherently deletes this grammatical scaffolding. Conversely, the Tail contains minimal semantic information, as evidenced by `Tail-Retention`'s floor performance (NDCG@10 = 0.0808).

While consistent across Qwen and Llama architectures, this study is limited to 2× compression on a single dataset. Future validation across other agglutinative languages and compression ratios is needed.
