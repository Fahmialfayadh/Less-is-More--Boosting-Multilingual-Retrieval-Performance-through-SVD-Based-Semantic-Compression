...
class QwenEmbedFilterPipeline:
    ...
    def encode_filtered(self, precomputed_raw_embs: torch.Tensor, filter_type: str = "english_middle", filter_ratio: int = 2) -> np.ndarray:
        """
        Applies linear projection post-hoc.
        filter_type: "english_middle" (384:1152), "indonesian_retention" (0:768), "tail_retention" (768:1536)
        """
        raw_embs = precomputed_raw_embs.to(self.device)
        
        if self.vh_matrix is not None:
            hidden_size = self.vh_matrix.shape[0]
            filter_num = hidden_size // filter_ratio # 768
            
            if filter_type == "english_middle":
                # Retain middle (384:1152) -> removing Head and Tail
                start_idx = (hidden_size - filter_num) // 2
                end_idx = start_idx + filter_num
                vh_sub = self.vh_matrix[start_idx:end_idx, :]
            elif filter_type == "indonesian_retention":
                # Retain Head+Middle (0:768)
                vh_sub = self.vh_matrix[0:filter_num, :]
            elif filter_type == "tail_retention":
                # Retain Tail (768:1536)
                vh_sub = self.vh_matrix[-filter_num:, :]
            else:
                raise ValueError("filter_type must be english_middle, indonesian_retention, or tail_retention")
...
with open('src/evaluate_retention_rag.py', 'w') as f: f.write(r'''
import os
import json
import random
...
def main():
    ...
    # 1. Baseline
    print("Evaluating Baseline (1536D)...")
    sts_base = eval_sts_custom(filter_type=None)
    rag_base = eval_rag_custom("run_baseline", filter_type=None)
    results.append({"config": "Baseline (1536D)", "sts_spearman": sts_base, **rag_base})

    # 2. English Middle (Edge-Filter)
    print("Evaluating English-Middle Retention (384:1152) -> 768D...")
    sts_en = eval_sts_custom(filter_type="english_middle")
    rag_en = eval_rag_custom("run_english_middle", filter_type="english_middle")
    results.append({"config": "English-Middle (768D)", "sts_spearman": sts_en, **rag_en})
    
    # 3. Indonesian Retention
    print("Evaluating Indonesian Retention Window (0:768) -> 768D...")
    sts_id = eval_sts_custom(filter_type="indonesian_retention")
    rag_id = eval_rag_custom("run_indonesian_retention", filter_type="indonesian_retention")
    results.append({"config": "Indonesian-Retention (768D)", "sts_spearman": sts_id, **rag_id})

    # 4. Tail Retention
    print("Evaluating Tail Retention (768:1536) -> 768D...")
    sts_tail = eval_sts_custom(filter_type="tail_retention")
    rag_tail = eval_rag_custom("run_tail_retention", filter_type="tail_retention")
    results.append({"config": "Tail-Retention (768D)", "sts_spearman": sts_tail, **rag_tail})
    
    with open("runs/metrics_retention.json", "w") as f:
...
with open('src/hypothesis_test_retention.py', 'w') as f: f.write(r'''
import os
...
def main():
    ...
    runs = []
    run_files = ["run_baseline.json", "run_english_middle.json", "run_indonesian_retention.json", "run_tail_retention.json"]
    ...
    print("\n" + "="*50)
    print("      RETENTION STATISTICAL SIGNIFICANCE REPORT")
    print("="*50)
    print(report)
...
run_cmd("python -u src/evaluate_retention_rag.py")

print("=== 3. Hypothesis Test ===")
run_cmd("python -u src/hypothesis_test_retention.py > runs/significance_retention.txt")
run_cmd("cat runs/significance_retention.txt")

run_cmd("cat runs/metrics_retention.json")
print("\nALL DONE.")
